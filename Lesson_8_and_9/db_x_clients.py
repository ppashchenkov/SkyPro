...
conn = psycopg2.connect(database="x_clients", user="x_clients_user", password="axcmq7V3QLCQwgL39GymqgasJhUlDbH4",
                        host="dpg-cl53o6ql7jac73cbgi2g-a.frankfurt-postgres.render.com", port=5432)
cur = conn.cursor()
...
def conn_connection():
    result = cur.execute("SELECT * FROM company where name = 'My New Company'")
    cur.fetchall()
    return result
